/**
 * Server-mode ApprovedWorkspaceStore.
 *
 * Same logic as the Electron version but takes userDataDir as a parameter
 * instead of calling app.getPath('userData').
 */
...
async function readSnapshot(filePath: string): Promise<ApprovedWorkspaceSnapshot | null> {
  try {
    ...
    if (version !== STORE_VERSION || !roots) return null
...
export function createServerApprovedWorkspaceStore(userDataDir: string): ApprovedWorkspaceStore {
  const storePath = resolve(userDataDir, 'approved-workspaces.json')
